...
if (
  window.location.href.indexOf('screen=ranking&mode=player') > -1 ||
  window.location.href.indexOf('screen=ranking&mode=con_player') > -1 ||
  window.location.href.indexOf('screen=ranking&mode=in_a_day') > -1 ||
  (window.location.href.indexOf('screen=ranking') > -1 && window.location.href.indexOf('&mode') == -1)
) {
  //any player
  tribeTable = '.vis:eq(2)'
  rowStart = 1
...
if (window.location.href.indexOf('screen=ranking&mode=kill_player') > -1) {
}
...
$('#contentContainer')
  .eq(0)
  .prepend("<span>BB</span><br><textarea id='BBCODE' rows='" + (rows + 4) + "' cols='100'></textarea><br>")
$('#contentContainer')
  .eq(0)
  .prepend("<span>CSV</span><br><textarea id='CSVCODE' rows='" + (rows + 4) + "' cols='100'></textarea><br>")
function getData() {
  ...
  for (var i = 0; i < names.length; i++) {
    ...
    if (statsEnabled.ODSTotal)
      linksODSTotal.push('/game.php?screen=ranking&mode=kill_player&type=support&name=' + names[i])
  }
  titleCount = 0
  for (var i = 0; i < Object.keys(statsEnabled).length; i++) {
    if (statsEnabled[Object.keys(statsEnabled)[i]]) {
      $(tribeTable + ' tr')
        .eq(rowStart - 1)
        .append(
          "<th onclick='sortTableTest(" +
            (columnStart + titleCount) +
            ")'>" +
            langShinko[Object.keys(langShinko)[i]] +
            '</th>',
        )
      titleCount++
    }
  ...
  $.getAll(
    ...
    () => {
      ...
      for (var o = rowStart; o < ODAperPlayer.length + rowStart; o++) {
        $(tribeTable + ' tr')
          .eq(o)
          .append('<td title=' + ODAperPlayer[o - rowStart][1] + '>' + ODAperPlayer[o - rowStart][0] + '</td>')
      }
      //ODD
      $.getAll(
        linksODD,
        ...
        () => {
          for (var o = rowStart; o < ODDperPlayer.length + rowStart; o++) {
            $(tribeTable + ' tr')
              .eq(o)
              .append('<td title=' + ODDperPlayer[o - rowStart][1] + '>' + ODDperPlayer[o - rowStart][0] + '</td>')
          }
          //ODS
          $.getAll(
            linksODS,
            ...
            () => {
              for (var o = rowStart; o < ODSperPlayer.length + rowStart; o++) {
                $(tribeTable + ' tr')
                  .eq(o)
                  .append('<td title=' + ODSperPlayer[o - rowStart][1] + '>' + ODSperPlayer[o - rowStart][0] + '</td>')
              }
                ...
                () => {
                  for (var o = rowStart; o < lootperPlayer.length + rowStart; o++) {
                    $(tribeTable + ' tr')
                      .eq(o)
                      .append(
                        '<td title=' + lootperPlayer[o - rowStart][1] + '>' + lootperPlayer[o - rowStart][0] + '</td>',
                      )
                  }
                  //gathering
                  $.getAll(
                    ...
                    () => {
                      for (var o = rowStart; o < gatheredperPlayer.length + rowStart; o++) {
                        $(tribeTable + ' tr')
                          ...
                          .append(
                            '<td title=' +
                              gatheredperPlayer[o - rowStart][1] +
                              '>' +
                              gatheredperPlayer[o - rowStart][0] +
                              '</td>',
                          )
                        ...
                        if (statsEnabled['Loot'] == true && statsEnabled['Gathering'] == true) {
                          $(tribeTable + ' tr')
                            ...
                            .append(
                              '<td>' +
                                numberWithCommas(
                                  parseInt(gatheredperPlayer[o - rowStart][0].split(',').join('')) +
                                    parseInt(lootperPlayer[o - rowStart][0].split(',').join('')),
                                ) +
                                '</td>',
                            )
                        } else {
                          if (statsEnabled['Combined'] == true)
                            $(tribeTable + ' tr')
                              .eq(o)
                              .append('<td>Need both Loot and Gathering enabled to see this column</td>')
                        }
                      }
                      // ODA total
                      $.getAll(
                        ...
                        () => {
                          for (var o = rowStart; o < ODATotalperPlayer.length + rowStart; o++) {
                            $(tribeTable + ' tr')
                              .eq(o)
                              .append('<td>' + numberWithCommas(ODATotalperPlayer[o - rowStart]) + '</td>')
                          }
                          ...
                          $.getAll(
                            ...
                            () => {
                              for (var o = rowStart; o < ODDTotalperPlayer.length + rowStart; o++) {
                                $(tribeTable + ' tr')
                                  .eq(o)
                                  .append('<td>' + numberWithCommas(ODDTotalperPlayer[o - rowStart]) + '</td>')
                              }
                              ...
                              $.getAll(
                                ...
                                () => {
                                  for (var o = rowStart; o < ODSTotalperPlayer.length + rowStart; o++) {
                                    $(tribeTable + ' tr')
                                      .eq(o)
                                      .append('<td>' + numberWithCommas(ODSTotalperPlayer[o - rowStart]) + '</td>')
                                  }
